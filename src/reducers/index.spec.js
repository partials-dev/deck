...
const startingCardId = rootReducer().gameDeck.cards[0].id
const updateCard = actions.updateCard({ id: startingCardId, size: cardSize })
const resetCardPosition = actions.resetCardPosition(startingCardId)
...
describe('root reducer', () => {
  ...
  describe('resetCardPosition()', () => {
    it('resets the specified card position', () => {
      const actionList = [updateScreen, updateCard, resetCardPosition]
      const finalState = applyActions(rootReducer, actionList)
      const isStartingCard = ({ id }) => id === startingCardId
      const startingCard = finalState.gameDeck.cards.find(isStartingCard)
      const expectedPosition = { x: 2, y: 1 }

      expect(startingCard.position).toEqual(expectedPosition)
      expect(startingCard.initialPosition).toEqual(expectedPosition)
    })
    it("throws an error if the card size hasn't been sent", () => {
      // don't set card size before trying to reset card position
      const actionList = [updateScreen, resetCardPosition]
      const applyInvalidActions = () => applyActions(rootReducer, actionList)
      expect(applyInvalidActions).toThrow('Card dimensions must have already been set.')
    })
    it("throws an error if the screen size hasn't been sent", () => {
      // don't set card size before trying to reset card position
      const actionList = [updateCard, resetCardPosition]
      const applyInvalidActions = () => applyActions(rootReducer, actionList)
      expect(applyInvalidActions).toThrow('Screen dimensions must have already been set.')
    })
  })
